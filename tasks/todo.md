...
- [ ] Parent module: adopt `resourceInput<>` types for same params (currently `object?`/`string`)
...
- [ ] Add example/test showing Entra ID auth using workload identity federation
- [ ] `authSettingV2Configuration` pre-filled with `identityProviders.azureActiveDirectory` using managed identity (no appReg client secret)
- [ ] Document in compliance metadata
