...
try {
    ...
    $results = (az network nsg list -g $groupName --query "[?contains(name, '$resourceFilter')].id") | ConvertFrom-Json
    if ($LastExitCode -gt 0) {
        Write-Output "Error looking for resources."
        Exit 1
    }

    Write-Output "Found $($results.Count) nsg resources containing '$resourceFilter'"

    foreach ($id in $results) {
        Write-Output "Removing NSG $id "
        if($whatIf -eq '') {
