...
@sys.description('The type to describe the scope-map configuration.')
@export()
type scopeMapsType = {
  @sys.description('Optional. The name of the scope map.')
  name: string?

  @sys.description('''Required. The list of scoped permissions for registry artifacts. Supported format is <resource type>/<resource name>/<resource action>.
  See https://aka.ms/acr/repo-permissions to find the supported resources and actions.''')
  actions: string[]
