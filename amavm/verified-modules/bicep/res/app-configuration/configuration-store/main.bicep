...
Available options for log categories are: 'HttpRequest', 'Audit'
''')
...
resource configurationStore_diagnosticSettings 'Microsoft.Insights/diagnosticSettings@2021-05-01-preview' = [
  for (diagnosticSetting, index) in (diagnosticSettings ?? []): {
    ...
    properties: {
      ...
      logs: [
        for group in (diagnosticSetting.?logCategoriesAndGroups ?? [{ category: 'HttpRequest' }, { category: 'Audit' }]): {
...
@batchSize(1)
module configurationStore_privateEndpoints 'br/amavm:res/network/private-endpoint:0.2.0' = [
  for (privateEndpoint, index) in (privateEndpoints ?? []): {
    name: '${uniqueString(deployment().name, location)}-configurationStore-pep-${index}'
...
@description('The type for the Key Value configuration.')
type appConfigKeyValuesType = {
@description('Required. Name of the configuration key.')
name: string
