...
@description('Optional. Indicates whether session affinity proxy is enabled.')
param clientAffinityProxyEnabled bool = true

@description('Optional. Indicates whether session affinity partitioning (CHIPS) is enabled.')
...
param functionAppConfiguration resourceInput<'Microsoft.Web/sites@2025-03-01'>.properties.functionAppConfig?
...
Prerequisites for FIC auth:
1. The app must have a managed identity (system-assigned or user-assigned)
2. The Entra ID app registration (identified by `authSettingApplicationId`) must have a federated identity credential trusting that managed identity
3. Set `appSettingsKeyValuePairs: { OVERRIDE_USE_MI_FIC_ASSERTION_CLIENTID: '<managed-identity-client-id>' }`
   For system-assigned identities, use the app's own client ID. For user-assigned, use the user-assigned MI's client ID.

To use traditional client secret auth instead, override via `authSettingV2ConfigurationAdditional`:
  `{ identityProviders: { azureActiveDirectory: { registration: { clientSecretSettingName: 'MICROSOFT_PROVIDER_AUTHENTICATION_SECRET' } } } }`

See: https://github.com/Azure-Samples/appservice-builtinauth-bicep for a complete example.
...
@description('Optional. Scope for auto-generated domain name label.')
...
module app_slots 'slot/main.bicep' = [
  for (slot, index) in (slots ?? []): {
    ...
    params: {
      ...
      managedEnvironmentResourceId: slot.?managedEnvironmentResourceId ?? managedEnvironmentId
      ...
      e2eEncryptionEnabled: slot.?e2eEncryptionEnabled ?? e2eEncryptionEnabled
      dnsConfiguration: slot.?dnsConfiguration ?? dnsConfiguration
      daprConfig: slot.?daprConfig ?? daprConfig
      sshEnabled: slot.?sshEnabled ?? sshEnabled
      ipMode: slot.?ipMode ?? ipMode
      resourceConfig: slot.?resourceConfig ?? resourceConfig
      workloadProfileName: slot.?workloadProfileName ?? workloadProfileName
      hostNamesDisabled: slot.?hostNamesDisabled ?? hostNamesDisabled
...
#disable-next-line outputs-should-not-contain-secrets // False positive: slotType.siteConfig uses resourceInput<> which flags azureStorageAccounts.*.accessKey, but slot names don't expose secrets
...
output slotSystemAssignedMIPrincipalIds array = [
  #disable-next-line outputs-should-not-contain-secrets // False positive: principal IDs don't expose secrets
...
#disable-next-line outputs-should-not-contain-secrets // False positive: PE outputs don't expose secrets
...
type slotType = {
  ...
  @description('Optional. The resource ID of the managed environment to use for this slot.')
  ...
  @description('Optional. Property to configure various DNS related settings for a slot.')
  ...
  @description('Optional. True if the app is reserved (Linux); otherwise, false.')
  reserved: bool?
