...
metadata compliance = '''Compliant usage of ServiceBus Namespace requires:
- minimumTlsVersion: TLS1_2
- disableLocalAuth: true
- publicNetworkAccess: 'Disabled'
- managedIdentity: not empty
'''
metadata complianceVersion = '20250428'
...
@description('''Optional. The number of partitions of a Service Bus namespace.

This property is only applicable to Premium SKU namespaces. The default value is 1 and possible values are 1, 2 and 4.''')
...
Available log categories: 'DiagnosticErrorLogs', 'OperationalLogs', 'VNetAndIPFilteringLogs', 'RuntimeAuditLogs', 'ApplicationMetricsLogs', 'AllMetrics'
''')
...
If Private endpoint configuration is not provided, the resource will be non-compliant.
''')
...
Note, this setting requires the configuration of Customer-Managed-Keys (CMK) via the corresponding module parameters.''')
...
resource avmTelemetry 'Microsoft.Resources/deployments@2024-03-01' = if (enableTelemetry) {
  name: take('${telemetryId}.res.servicebus-namespace.${replace('-..--..-', '.', '-')}.${substring(uniqueString(deployment().name, location), 0, 4)}', 64)
  properties: {
    mode: 'Incremental'
    template: {
...
resource serviceBusNamespace_diagnosticSettings 'Microsoft.Insights/diagnosticSettings@2021-05-01-preview' = [
  for (diagnosticSetting, index) in (diagnosticSettings ?? []): {
    ...
    properties: {
      ...
      logs: [
        for group in (diagnosticSetting.?logCategoriesAndGroups ?? [{ category: 'DiagnosticErrorLogs' }, { category: 'OperationalLogs' }, { category: 'VNetAndIPFilteringLogs' }, { category: 'RuntimeAuditLogs' }, { category: 'ApplicationMetricsLogs' }]): {
...
module serviceBusNamespace_privateEndpoints 'br/amavm:res/network/private-endpoint:0.2.0' = [
  for (privateEndpoint, index) in (privateEndpoints ?? []): {
    name: '${uniqueString(deployment().name, location)}-serviceBusNamespace-pep-${index}'
    scope: resourceGroup(privateEndpoint.?resourceGroupName ?? '')
    params: {
      name: privateEndpoint.?name ?? '${last(split(serviceBusNamespace.id, '/'))}-pep-${privateEndpoint.?service ?? 'namespace'}-${index}'
...
type networkRuleSetType = {
  ...
  @description('''Optional. Default Action for Network Rule Set. Default is "Allow". 
  It will not be set if publicNetworkAccess is "Disabled". Otherwise, it will be set to "Deny" if ipRules or virtualNetworkRules are being used.''')
...
type queueType = {
  ...
  @description('''Optional. A value that indicates whether Express Entities are enabled.
  An express queue holds a message in memory temporarily before writing it to persistent storage.
  This property is only used if the 'service-bus/namespace' sku is Premium.''')
  enableExpress: bool?
  ...
  @description('''Optional. ISO 8601 timespan duration of a peek-lock; that is, the amount of time that the message is locked for other receivers.
...
type topicType = {
  ...
  @description('''Optional. A value that indicates whether Express Entities are enabled.
  An express topic holds a message in memory temporarily before writing it to persistent storage. This property is only used if the `service-bus/namespace` sku is Premium.''')
  enableExpress: bool?
