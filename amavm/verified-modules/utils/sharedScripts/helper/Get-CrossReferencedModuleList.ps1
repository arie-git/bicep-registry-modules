...
Invoke the function with the default path. Returns an object such as:
{
    "Compute/availabilitySets": {
        "remoteReferences": [
            "bicep-res-network-networkinterface"
        ],
        "resourceReferences": [
...
function Get-CrossReferencedModuleList {
    ...
    param (
        [Parameter()]
        [string] $Path = (Join-Path (Get-Item $PSScriptRoot).Parent.Parent.Parent "bicep/")
    )

    $repoRoot = ($Path -split '[\/|\\]{1}bicep[\/|\\]{1}')[0]
    $resultSet = [ordered]@{}
    ...
    foreach ($moduleTemplatePath in $moduleTemplatePaths) {
        ...
        $resourceTypeIdentifier = ($moduleFolderPath -split '[\/|\\]bicep[\/|\\](res|ptn|utl)[\/|\\]')[2] -replace '\\', '/'
