...
function Set-DefinitionSection {
    ...
    foreach ($category in $sortedParamCategories) {
        ...
        foreach ($parameter in $categoryParameters) {
            ...
            if (($parameter.name -eq 'roleAssignments') -and ($TemplateFileContent.variables.keys -contains 'specificBuiltInRoleNames')) {
                if ([String]::IsNullOrEmpty($ParentName)) {
                    # Top-level invocation
                    $roles = $TemplateFileContent.variables.specificBuiltInRoleNames.Keys
                } else {
                    ...
                    if ($resourceIdentifier = $flattendResources.identifier | Where-Object { $_ -match "^.*_$ParentName`$" }) {
                        $roles = ($flattendResources | Where-Object {
                            ...
                            }).properties.template.variables.specificBuiltInRoleNames.Keys
                    } else {
...
function Set-UsageExamplesSection {
    ...
    $SectionContent = [System.Collections.ArrayList]@(
        ...
        '',
        ('>**Note**: To reference the module, please use the following syntax `br/<registry-alias>:{0}:{1}`.' -f $brLink, $targetVersion),
        ''
    )
    ...
    $testFilePaths | ForEach-Object -Parallel {
        ...
        $builtTemplate = (az bicep build --file $_ --stdout 2>$null) | Out-String

        if ([String]::IsNullOrEmpty($builtTemplate)) {
            throw "Failed to build template [$_]. Try running the command ``az bicep build --file $_ --stdout`` locally for troubleshooting. Make sure you have the latest Bicep CLI installed."
        }
        ...
        if ($rawContentArray -match $isModuleDeploymentRegex) {
            ...
            if ($addBicep) {
                ...
                $formattedBicepExample = @(
                    "module $($moduleNameCamelCase)Mod 'br/<registry-alias>:$($brLink):$($targetVersion)' = {",
                    "  name: '$($moduleNameCamelCase)-mod'"
                    '  params: {'
                ) + $bicepExample +
                @( '  }',
                    '}'
...
function Initialize-ReadMe {
    ...
    # Two lines below are APG customization
    $moduleCompliance = $TemplateFileContent.metadata.compliance
    $moduleComplianceVersion = "Version: " + $TemplateFileContent.metadata.complianceVersion

    if ($ReadMeFilePath -match 'bicep.(?:res)') {
        # Resource module
...
function Set-ModuleReadMe {
    ...
    . (Join-Path $PSScriptRoot 'sharedScripts' 'helper' 'Get-NestedResourceList.ps1')
    . (Join-Path $PSScriptRoot 'sharedScripts' 'helper' 'Merge-FileWithNewContent.ps1')
    . (Join-Path $PSScriptRoot 'sharedScripts' 'helper' 'Get-IsParameterRequired.ps1')
    . (Join-Path $PSScriptRoot 'sharedScripts' 'helper' 'Get-SpecsAlignedResourceName.ps1')
    . (Join-Path $PSScriptRoot 'sharedScripts' 'helper' 'ConvertTo-OrderedHashtable.ps1')
    . (Join-Path $PSScriptRoot 'sharedScripts' 'Get-BRMRepositoryName.ps1')
    . (Join-Path $PSScriptRoot 'sharedScripts' 'helper' 'Get-CrossReferencedModuleList.ps1')
    ...
    if ($PreLoadedContent.Keys -notcontains 'TemplateFileContent') {
        if ((Split-Path -Path $TemplateFilePath -Extension) -eq '.bicep') {
            $templateFileContent = az bicep build --file $TemplateFilePath --stdout 2>$null | ConvertFrom-Json -AsHashtable
        } else {
    ...
    $fullModuleIdentifier = ($moduleRoot -split '[\/|\\]bicep[\/|\\](res|ptn|utl)[\/|\\]')[2] -replace '\\', '/'
